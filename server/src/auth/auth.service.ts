...
export class AuthService {
  ...
  async googleLogin(email: string) {
    ...
    if (result.status == 2) {
      const { access, refresh } = await this.getTokens(result.user.userIdx);
      return {
        access,
        refresh,
        result,
  ...
  async getTokens(userIdx: number) {
    const { accessToken, ...accessOption } =
      this.getCookieWithJwtAccessToken(userIdx);
    const { refreshToken, ...refreshOption } =
      this.getCookieWithJwtRefreshToken(userIdx);
